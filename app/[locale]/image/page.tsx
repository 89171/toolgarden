...
export default async function ImageHubPage({
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground">
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: toJsonLd(createHubFaqJsonLd('image_hub', normalizedLocale)) }}
      />
      <div className="flex w-full flex-grow flex-col px-3 py-3 sm:px-6 sm:py-4 lg:px-10 xl:px-14 2xl:px-20">
        ...
        <header className="mb-8">
          ...
          <h1 className="text-2xl font-bold leading-tight text-content sm:text-3xl">{m.image_hub.title}</h1>
        ...
        <HubFaqSection items={getHubFaqItems('image_hub', normalizedLocale)} title={m.blog.faq_title} />
      </div>
      <Footer />
    </div>
  );
