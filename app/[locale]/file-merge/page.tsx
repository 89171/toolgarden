...
export default async function FileMergeHubPage({
  ...
  const fileMergeToolIds = new Set(getFileMergeTools().map((tool) => tool.id));
  const fileMergeTools = getLocalizedToolCards(normalizedLocale).filter((tool) => fileMergeToolIds.has(tool.id));
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-background text-foreground">
      ...
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: toJsonLd(createHubFaqJsonLd('file_merge_hub', normalizedLocale)) }}
      />
      <div className="flex w-full flex-grow flex-col px-3 py-3 sm:px-6 sm:py-4 lg:px-10 xl:px-14 2xl:px-20">
        ...
        <header className="mb-8">
          ...
          <h1 className="text-2xl font-bold leading-tight text-content sm:text-3xl">{m.file_merge_hub.title}</h1>
          <p className="mt-3 max-w-3xl text-base leading-relaxed text-content-muted">
            {m.file_merge_hub.description}
          </p>
          <p className="mt-2 text-sm text-content-faint">{m.file_merge_hub.privacy_note}</p>
        </header>

        <section>
          <h2 className="mb-4 border-b border-border-subtle pb-2 text-xs font-semibold uppercase tracking-normal text-content-faint">
            {m.file_merge_hub.all_tools}
          </h2>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
            {fileMergeTools.map((tool) => (
              <Link
                key={tool.id}
        ...
        <HubFaqSection items={getHubFaqItems('file_merge_hub', normalizedLocale)} title={m.blog.faq_title} />
      </div>
      <Footer />
    </div>
  );
