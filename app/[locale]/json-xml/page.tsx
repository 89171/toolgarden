...
export default function JsonXmlPage() {
  ...
  const convert = (fn: (s: string) => { ok: true; output: string } | { ok: false; message: string }, from: string, setSide: (v: string) => void) => {
    const r = fn(from); if (r.ok) { setSide(r.output); setError(''); } else { setError(r.message); }
  };
    ...
    <ToolLayout toolId="json-xml">
      ...
      <div className="flex-grow grid grid-cols-1 lg:grid-cols-2 gap-6 min-h-0">
        <Panel title="JSON" actions={<>
          <Button onClick={() => convert(jsonToXml, left, setRight)}>{t('to_xml')}</Button>
        ...
        <Panel title="XML" actions={<>
          <Button onClick={() => convert(xmlToJson, right, setLeft)}>{t('to_json')}</Button>
