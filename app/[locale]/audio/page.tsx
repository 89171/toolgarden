...
export default async function AudioHubPage({
  ...
  const audioToolIds = new Set(getAudioTools().map((tool) => tool.id));
  const audioTools = getLocalizedToolCards(normalizedLocale).filter((tool) => audioToolIds.has(tool.id));
      ...
      <div className="flex w-full flex-grow flex-col px-3 py-3 sm:px-6 sm:py-4 lg:px-10 xl:px-14 2xl:px-20">
        ...
        <header className="mb-8">
          <nav className="mb-4 flex items-center gap-1 text-sm text-content-muted" aria-label="breadcrumb">
            ...
            <span>/</span>
            <span className="font-medium text-content-secondary">{m.audio_hub.breadcrumb}</span>
          </nav>
          <h1 className="text-2xl font-bold leading-tight text-content sm:text-3xl">{m.audio_hub.title}</h1>
          <p className="mt-3 max-w-3xl text-base leading-relaxed text-content-muted">
            {m.audio_hub.description}
          </p>
          <p className="mt-2 text-sm text-content-faint">{m.audio_hub.privacy_note}</p>
        </header>

        <section>
          <h2 className="mb-4 border-b border-border-subtle pb-2 text-xs font-semibold uppercase tracking-normal text-content-faint">
            {m.audio_hub.all_tools}
          </h2>
