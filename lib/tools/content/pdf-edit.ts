...
export const pdfEditContent = defineToolContent({
  zh: {
    ...
    steps: [
      ...
      ['改原文：点击虚线框里的文字', '选「改原文」后页面上的文字块会显示虚线框，点击即可输入新内容，回车确认，改写会排队列出。'],
    ...
    notes: [
      ...
      '三种情况无法改写原文，工具会在导出后逐条说明：文字不在页面内容流里（例如在表单域或 Form XObject 中）、字体是多字节 CID 字体、原字体里没有新文字需要的字形（中文子集字体常见）。',
      '填充方框只是盖住视觉内容，被盖住的文字仍在文字层里可被复制提取，不能当作脱敏涂黑使用；真正要删掉的文字请用改原文。',
      '导出会重写文件，已有的数字签名会失效；加密 PDF 需要先解除密码再编辑。',
    ],
    specs: [
      ['改原文的实现', '定位并替换页面内容流里的 Tj / TJ 字符串，不是盖图层，结果仍是真正的文字'],
      ['改原文的前提', '文字在页面自己的内容流里、字体为单字节字体，且新文字每个字符在原字体中有字形'],
      ['失败时的行为', '明确报出原因并跳过，绝不写出渲染成空白或豆腐块的 PDF'],
  ...
  en: {
    ...
    steps: [
      ...
      ['Rewrite: click a dashed text box', 'In Rewrite text mode every text run gets a dashed outline. Click one, type the new content, press Enter, and the change is queued.'],
    ...
    notes: [
      ...
      'Three cases cannot be rewritten, and each is reported after export: the text is not in the page content stream (a form field or Form XObject, for example), the font is a multi-byte CID font, or the original font has no glyph for the new characters — common with subset CJK fonts.',
    ...
    specs: [
      ['How rewriting works', 'The Tj / TJ strings in the page content stream are located and replaced, so the result is real text, not a stamped layer'],
      ...
      ['Behavior on failure', 'The run is skipped and the reason reported; the tool never writes a PDF that renders as blanks or tofu boxes'],
