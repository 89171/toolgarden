...
export const pdfWatermarkContent = defineToolContent({
  zh: {
    ...
    specs: [["水印类型", "文字水印，可设置九宫格定位、平铺、对角线排布和透明度"], ["叠加方式", "在原页面之上新增一层内容，原页面对象不被改写"], ["能防篡改吗", "不能。水印是可被其它 PDF 工具再次编辑或移除的图层，它是标识而不是保护"], ["对文字层的影响", "不影响原文档的文字层，加水印后仍可选中和复制正文"], ["适用范围", "作用于全部页面，页面尺寸不一致时水印位置按各页自身尺寸计算"], ["需要真正保护时", "请改用 PDF 加密设置打开密码"]],
